...
class Fmeasure(object):
    ...
    def cal_pr(self, pred: np.ndarray, gt: np.ndarray) -> tuple:
        ...
        # 1. 실제 값 이전의 배경 영역에서 예측된 결과의 히스토그램 가져오기
        ...
        # 2. 누적 히스토그램을 사용하여 배경에서 해당 실제 값 이전의 다른 임계값보다 큰 픽셀 수를 얻습니다.
        # 여기서는 누적 히스토그램을 사용하여 한 번에 다른 임계값보다 큰 픽셀 수를 구하며, 여기서는 전경 영역만 계산합니다.
        ...
        # 3. 서로 다른 임계값의 결과를 사용하여 해당 정밀도 및 회수율을 계산합니다.
        # p 및 r 계산의 실제 값은 pred==1 및 gt==1이며, 분모만 다르며 전자의 경우 pred==1, 후자의 경우 gt==1입니다.
        # 서로 다른 임계값에 대한 결과를 동시에 계산하기 위해, 여기서는 hsitogram&flip&cumsum을 사용하여 서로 다른 각각의 전경 픽셀 수를 얻습니다.
        TPs = fg_w_thrs
        Ps = fg_w_thrs + bg_w_thrs

        # 为防止除0，这里针对除0的情况分析后直接对于0分母设为1，因为此时分子必为0
        # 0으로 나눗셈을 방지하려면 분자가 0이어야 하므로 0으로 나눗셈에 대한 분석 후 분모를 0에 대해 바로 1로 설정합니다.
        Ps[Ps == 0] = 1
        ...
        #  但是这里使用TPs不便于处理列表
        # 특정 임계값에서 T=0 또는 fg_w_thrs=0 또는 bg_w_thrs=0인 경우 TPs[i]=0에 포함될 수 있습니다.
        # 하지만 여기서 TP를 사용하면 목록 처리가 용이하지 않습니다.
        precisions = TPs / Ps
        recalls = TPs / T
